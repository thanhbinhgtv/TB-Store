...
export function ItemListComponent({ items }: ItemListProps) {
  return (
    ...
    >
      {/* <h2 className="text-xl font-semibold text-[var(--text-accent)]">Sản phẩm bán chạy</h2> */}
          ...
          >
            {index === 0 ? (
              <div className="space-y-3">
                <div className="aspect-[4/4] animate-pulse rounded-lg bg-[var(--surface-muted)]" />
                <div className="h-4 w-3/4 animate-pulse rounded bg-[var(--surface-muted)]" />
                <div className="h-3 w-1/2 animate-pulse rounded bg-[var(--surface-subtle)]" />
                <div className="h-9 w-full animate-pulse rounded-lg bg-[var(--surface-muted)]" />
                <div className="font-bold text-[var(--text-accent)]">{item.feature}</div>
              </div>
            ) : (
              <div
